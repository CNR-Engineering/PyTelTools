...
from workflow.util import MultiLoadSerafinDialog, MultiSaveDialog, VolumePlotViewer, \
    ...
    MultiSaveMultiVarLinePlotDialog, MultiSaveMultiFrameLinePlotDialog
...
class ProjectLinesNode(DoubleInputNode):
    ...
    def multi_save(self):
        ...
        compute_options = (reference, max_distance, time_index, current_vars)
...
class MultiVarLinePlotNode(DoubleInputNode):
    ...
    def compute(self, time_index, input_data,
    ...
    def multi_save(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            return
        dlg = MultiSaveDialog(['_multi_var_line_plot', True, '', False, True])
        dlg.panel.no_button.setEnabled(False)
        if dlg.exec_() == QDialog.Accepted:
            output_options = dlg.panel.get_options()
        else:
        ...
        time_index = int(self.plot_viewer.control.timeSelection.index.text()) - 1
        compute_options = (line_id, time_index, current_vars)
...
class MultiFrameLinePlotNode(DoubleInputNode):
    ...
    def compute(self, input_data,
                line_interpolator, line_interpolator_internal, current_var, time_indices):
        values = []
        values_internal = []
        with Serafin.Read(input_data.filename, input_data.language) as input_stream:
            input_stream.header = input_data.header
    ...
    def multi_save(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            return
        dlg = MultiSaveDialog(['_multi_frame_line_plot', True, '', False, True])
        dlg.panel.no_button.setEnabled(False)
        if dlg.exec_() == QDialog.Accepted:
            output_options = dlg.panel.get_options()
        else:
        ...
        compute_options = (line_id, current_var, time_indices)
