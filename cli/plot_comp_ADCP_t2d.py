...
def Plot_ADCP_TELEMAC(args):
    ...
    with open(args.inTELEMAC) as dirTel:
        ...
        for dirfor in folder:
            for filename in glob.glob(os.path.join(dirfor, '*.csv')):
                ...
                col_labels.append(os.path.basename(os.path.dirname(dirfor)))
                ...
                axs[0].plot(file_df_mean_TEL['Distance'], file_df_mean_TEL['value'], label=("Mean"+os.path.basename(os.path.dirname(dirfor))))
...
parser.add_argument("inTELEMAC", help="List of folder containing (.csv) files")
parser.add_argument("--Numberdiv", help="Segments number of the line to compute average velocity on a normal grid",default=10)
parser.add_argument("outGraph", help="Filename of plot (.png)")

if __name__ == "__main__":
    args = parser.parse_args()
    Plot_ADCP_TELEMAC(args)